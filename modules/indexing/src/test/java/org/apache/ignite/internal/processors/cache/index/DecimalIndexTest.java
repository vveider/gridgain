...
public class DecimalIndexTest extends AbstractIndexingCommonTest {
    ...
    public void correctInlineSIze() throws Exception {
        ...
        assertFalse("Unexpected full inline of the index", lsnr.check());
    }
    ...
    public void nullAndOutOfRangeValues() throws Exception {
        ...
        sql(node, "insert into " + TBL_NAME + "(key_pk, key1, key2, val) values (1, 1.0, 2.0, 1)");
        sql(node, "insert into " + TBL_NAME + "(key_pk, key1, key2, val) values (2, null, 2.0, 1)");
        sql(node, "insert into " + TBL_NAME + "(key_pk, key1, key2, val) values (3, 1.0, null, 1)");
        sql(node, "insert into " + TBL_NAME + "(key_pk, key1, key2, val) values (4, 3.40282347E+99, 2.0, 1)");
        sql(node, "insert into " + TBL_NAME + "(key_pk, key1, key2, val) values (5, 1.0, 3.40282347E+99, 1)");
        sql(node, "insert into " + TBL_NAME + "(key_pk, key1, key2, val) values (6, 1.0, 2.0, 1)");

        assertEquals(6, sql(node,"select * from " + TBL_NAME).size());
    }
